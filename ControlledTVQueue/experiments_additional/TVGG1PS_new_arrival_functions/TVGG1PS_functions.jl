...
#function set_tables_general_function(TVAS::Any, TVSS::Any)
#    TVAS.table = table_for_sinusoidal_J((TVAS.α,TVAS.β,TVAS.γ))
#    TVSS.table = table_for_any_periodic_J(TVSS.μ, 2*π/TVAS.γ)
#end

function set_tables_general_function(TVAS::Any, TVSS::Any, period::Any)
    TVAS.table = table_for_any_periodic_J(TVAS.λ, period)
    TVSS.table = table_for_any_periodic_J(TVSS.μ, period)
end
...
#    g_e = t -> 1-cdf(TVAS.base_distribution,t)/mean(TVAS.base_distribution)
#    S = inverse_cdf(g_e , rand())
    g = t -> pdf(TVAS.base_distribution,t)
    S = inverse_cdf(g , rand())
  	V = [ inverse_integrated_rate_function(TVAS.Λ, former_last_arrival_time, S, TVAS.table) ]
#  	V = [ inverse_integrated_rate_function(TVAS.Λ, former_last_arrival_time, S) ]
    n = 1
...
#      system.next_completion_time = inverse_integrated_rate_function(system.TVSS.M, system.sim_time, Q*system.WIP[nci].remaining_workload)
  ...
  elseif system.next_regular_recording == min(system.next_arrival_time, system.next_completion_time, system.next_regular_recording)
    ...
    record.P[round(current_time,5)] = deepcopy(TVGG1PS_queue_path(system))
...
#        path.next_completion_time = inverse_integrated_rate_function(system.TVSS.M, path.sim_time, Q*path.WIP[nci].remaining_workload)
      end
...
function run_to_end(system::TVGG1PS_queue, customer_pool::Array{Customer}, record::Record)
    for t in record.T
#        println("running $t")
        sojourn_time = simulateSojournTime(system, customer_pool, record.P[t])
        push!(record.S, sojourn_time) # 시점 t의 Path에 Virtual Customer 하나 넣고 다시 시뮬레이션 go
    end
end
...
function do_experiment(queue::String, control::String, target::Float64, arrival::String, service::String, arrival_function::Function, period::Any, fcn_type::String, T::Float64, N::Int64, record::Record, LOG_PATH::String)
    TVAS = Time_Varying_Arrival_Setting(arrival_function, arrival)
    ...
    set_tables_general_function(TVAS, TVSS, period)
    file_num_in_queue = open("$LOG_PATH/$(queue)_$(target)_$(control)_$(arrival)_$(service)_$(fcn_type)_$(T)_$(N)_queue_length.txt" , "w")
    file_virtual_sojourn_time = open("$LOG_PATH/$(queue)_$(target)_$(control)_$(arrival)_$(service)_$(fcn_type)_$(T)_$(N)_sojourn_time.txt" , "w")

    running_mark = open("$LOG_PATH/$(queue)_$(target)_$(control)_$(arrival)_$(service)_$(fcn_type)_$(T)_$(N)_running.txt", "w")
    ...
    rm("$LOG_PATH/$(queue)_$(target)_$(control)_$(arrival)_$(service)_$(fcn_type)_$(T)_$(N)_running.txt")
    completion_mark = open("$LOG_PATH/$(queue)_$(target)_$(control)_$(arrival)_$(service)_$(fcn_type)_$(T)_$(N)_completed.txt", "w")
    close(completion_mark)

    gc()
end

function do_experiment_table(queue::String, control::String, target::Float64, arrival::String, service::String, fcn, fcn_type, arrival_table, service_table, T::Float64, N::Int64, record::Record, LOG_PATH::String)
    TVAS = Time_Varying_Arrival_Setting(fcn, arrival)
    TVSS = Time_Varying_Service_Setting(TVAS, control, target, service)
    set_distribution(TVAS)
    set_distribution(TVSS)
    set_service_rate_function(TVAS, TVSS)
#    set_tables_general_function(TVAS, TVSS, period)
    TVAS.table = arrival_table
    TVSS.table = service_table
    file_num_in_queue = open("$LOG_PATH/$(queue)_$(target)_$(control)_$(arrival)_$(service)_$(fcn_type)_$(T)_$(N)_queue_length.txt" , "w")
    file_virtual_sojourn_time = open("$LOG_PATH/$(queue)_$(target)_$(control)_$(arrival)_$(service)_$(fcn_type)_$(T)_$(N)_sojourn_time.txt" , "w")
...
function do_experiment_table_slave(queue::String, control::String, target::Float64, arrival::String, service::String, arrival_function::Function, arrival_table, service_table, T::Float64, N::Int64, record::Record, LOG_PATH::String, slave_num::Int64)
    TVAS = Time_Varying_Arrival_Setting(arrival_function, arrival)
    TVSS = Time_Varying_Service_Setting(TVAS, control, target, service)
    set_distribution(TVAS)
    set_distribution(TVSS)
...
#    set_tables_general_function(TVAS, TVSS, period)
    TVAS.table = arrival_table
    TVSS.table = service_table
    file_num_in_queue = open("$LOG_PATH/$(queue)_$(target)_$(control)_$(arrival)_$(service)_$(fcn_type)_$(T)_$(N)_queue_length_$(slave_num).txt" , "w")
    file_virtual_sojourn_time = open("$LOG_PATH/$(queue)_$(target)_$(control)_$(arrival)_$(service)_$(fcn_type)_$(T)_$(N)_sojourn_time_$(slave_num).txt" , "w")

    running_mark = open("$LOG_PATH/$(queue)_$(target)_$(control)_$(arrival)_$(service)_$(fcn_type)_$(T)_$(N)_running_$(slave_num).txt", "w")
    ...
    rm("$LOG_PATH/$(queue)_$(target)_$(control)_$(arrival)_$(service)_$(fcn_type)_$(T)_$(N)_running_$(slave_num).txt")
    completion_mark = open("$LOG_PATH/$(queue)_$(target)_$(control)_$(arrival)_$(service)_$(fcn_type)_$(T)_$(N)_completed_$(slave_num).txt", "w")
    close(completion_mark)
...
function do_experiment(queue::String, control::String, target::Float64, arrival::String, service::String, coeff::Tuple, T::Float64, N::Int64, record::Record, LOG_PATH::String)
    ...
    for n in 1:N
      ...
      record.S = Float64[]
      record.P = Dict{Float64, TVGG1PS_queue_path}()
      customer_pool = generate_customer_pool(TVAS, TVSS, T*3.0)
      defaultize(system)
      system.next_arrival_time = customer_pool[1].arrival_time
      system.size_customer_pool = length(customer_pool)
      storePath(system, customer_pool, record, T)
