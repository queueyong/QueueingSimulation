...
function do_experiment_table_slave(queue::String, control::String, target::Float64, arrival::String, service::String, fcn, fcn_type, arrival_table, service_table, T::Float64, N::Int64, record::Record, LOG_PATH::String, k::Int64)
    TVAS = Time_Varying_Arrival_Setting(fcn, arrival)
    TVSS = Time_Varying_Service_Setting(TVAS, control, target, service)
    set_distribution(TVAS)
    set_distribution(TVSS)
...
#    set_tables_general_function(TVAS, TVSS, period)
    TVAS.table = arrival_table
    TVSS.table = service_table
    file_num_in_queue = open("$LOG_PATH/$(queue)_$(target)_$(control)_$(arrival)_$(service)_$(fcn_type)_$(T)_$(N)_queue_length_slave$k.txt" , "w")
    file_virtual_sojourn_time = open("$LOG_PATH/$(queue)_$(target)_$(control)_$(arrival)_$(service)_$(fcn_type)_$(T)_$(N)_sojourn_time_slave$k.txt" , "w")

    running_mark = open("$LOG_PATH/$(queue)_$(target)_$(control)_$(arrival)_$(service)_$(fcn_type)_$(T)_$(N)_slave$(k)_running.txt", "w")
    ...
    rm("$LOG_PATH/$(queue)_$(target)_$(control)_$(arrival)_$(service)_$(fcn_type)_$(T)_$(N)_slave$(k)_running.txt")
    completion_mark = open("$LOG_PATH/$(queue)_$(target)_$(control)_$(arrival)_$(service)_$(fcn_type)_$(T)_$(N)_slave$(k)_completed.txt", "w")
    close(completion_mark)
